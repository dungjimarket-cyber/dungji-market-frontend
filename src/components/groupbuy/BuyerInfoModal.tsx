...
import { Phone, User, Calendar, CheckCircle, XCircle, Clock, Package, AlertCircle } from 'lucide-react';
...
export function BuyerInfoModal({ isOpen, onClose, groupBuyId }: BuyerInfoModalProps) {
  ...
  useEffect(() => {
    ...
    fetchBuyersInfo();
  }, [groupBuyId, isAuthenticated, accessToken, user, isOpen]);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold">구매자 정보</DialogTitle>
          {groupBuy && (
            <DialogDescription>
              {groupBuy.title} - {groupBuy.product_name}
            </DialogDescription>
          )}
        </DialogHeader>
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-4">
                <div className="flex items-center gap-4 text-sm text-gray-600">
                  <span className="flex items-center gap-1">
                    ...
                    구매확정: <strong>{groupBuy.confirmed_buyers_count}명</strong>
                  </span>
                  <span className="flex items-center gap-1">
                    <Package className="w-4 h-4" />
                    전체 참여: {groupBuy.total_participants}명
                  </span>
                </div>
              </div>
            )}
                        ...
                        <>
                          {buyer.user.phone && (
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <span className="text-sm font-medium">{buyer.user.phone}</span>
                            </div>
                          )}
