...
export default function CustomNoShowReportsMade() {
  const { user, accessToken } = useAuth();
  const router = useRouter();
  const [reports, setReports] = useState<CustomNoShowReport[]>([]);
  const [loading, setLoading] = useState(true);
  const [cancelDialogOpen, setCancelDialogOpen] = useState(false);
  const [selectedReportId, setSelectedReportId] = useState<number | null>(null);
  const [selectedReport, setSelectedReport] = useState<CustomNoShowReport | null>(null);
  const [editDialogOpen, setEditDialogOpen] = useState(false);
  const [editingReport, setEditingReport] = useState<CustomNoShowReport | null>(null);
  ...
  const fetchReports = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/custom-noshow-reports/?type=made`,
        {
          headers: {
  ...
  const handleCancelReport = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/custom-noshow-reports/${selectedReportId}/cancel/`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${accessToken}`,
  ...
  const checkCanEdit = async (reportId: number) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/custom-noshow-reports/${reportId}/can_edit/`,
        {
          headers: {
            'Authorization': `Bearer ${accessToken}`,
          },
  ...
  const openEditDialog = async (report: CustomNoShowReport) => {
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/custom-noshow-reports/${editingReport.id}/`,
        {
          method: 'PATCH',
          headers: {
            'Authorization': `Bearer ${accessToken}`,
  ...
  const openCancelDialog = (report: CustomNoShowReport) => {
    ...
    return (
      <Card>
        <CardContent className="text-center py-12">
          ...
          <p className="text-sm text-gray-500 mt-2">마감된 커스텀 공구에서 노쇼 신고가 가능합니다.</p>
        </CardContent>
      </Card>
    );
  }
          ...
          <Card key={report.id}>
            <CardHeader>
              <div className="flex justify-between items-start">
                <div>
                  <CardTitle className="text-lg">{report.custom_groupbuy_title}</CardTitle>
