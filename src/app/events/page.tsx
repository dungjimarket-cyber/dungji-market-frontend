      ...
      {events.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 sm:gap-3 lg:gap-4">
          {events.map((event) => {
            ...
            const EventWrapper = isActive ? Link : 'div';

            return (
              <EventWrapper
                key={event.id}
                {...(isActive ? { href: `/events/${event.slug}` } : {})}
                className={`bg-white rounded-lg shadow-sm overflow-hidden transition-all duration-200 transform scale-90 lg:scale-[0.85] xl:scale-[0.8] ${
              ...
              >
                ...
                <div className="p-2 sm:p-3">
                  <h2 className={`text-sm sm:text-base font-semibold mb-1 line-clamp-2 ${!isActive ? 'text-gray-500' : ''}`}>
                    {event.title}
                  </h2>
