...
export async function GET(request: NextRequest) {
  ...
  try {
    // Django 백엔드의 reminder endpoint 호출
    const response = await fetch(`${BACKEND_API_URL}/api/cron/send-reminders/`, {
      method: 'POST',
      headers: {
    ...
    console.log('[Cron] Reminder notifications sent:', data);
    
    return NextResponse.json({
      success: true,
      message: 'Reminder notifications cron job completed',
      data
    });
  } catch (error) {
