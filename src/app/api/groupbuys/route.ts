...
export async function GET(request: NextRequest) {
  try {
    ...
    // 상태 계산 및 업데이트
    const updatedGroupBuys = mockGroupBuys.map(groupBuy => {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 필수 필드 검증
    const requiredFields = ['title', 'product', 'max_participants', 'min_participants', 'end_time'];
    ...
    return NextResponse.json(
      { 
        ...
        ...data,
        status: 'recruiting',
        current_participants: 1, // 생성자가 자동으로 참여
