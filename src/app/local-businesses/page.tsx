...
function LocalBusinessesContent() {
  ...
  return (
    <>
      ...
      {/* 플로팅 상담신청 버튼 - 채팅 버블 스타일 */}
      <button
        onClick={() => setConsultModalOpen(true)}
        className="fixed z-40 bg-cyan-500 hover:bg-cyan-600 text-white shadow-lg hover:shadow-xl transition-all duration-200 flex items-center gap-2 hover:scale-105 active:scale-95
          right-4 bottom-24 px-4 py-3 rounded-full
          sm:right-auto sm:left-1/2 sm:-translate-x-1/2 sm:bottom-8 sm:px-6 sm:py-4 sm:rounded-[28px] sm:rounded-br-md"
        aria-label="무료상담신청"
      >
        <MessageCircle className="w-5 h-5 sm:w-6 sm:h-6" />
        <span className="text-base sm:text-lg whitespace-nowrap font-[var(--font-black-han-sans)]" style={{ fontFamily: 'var(--font-black-han-sans)' }}>무료상담</span>
      </button>
